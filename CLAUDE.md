...
- **`Rodar.Event.End`** — End event handler. Plain end returns `{:ok, context}`. Error end sets error state. Terminate end marks process terminated. Cancel end triggers transaction cancellation and scoped compensation. Message/signal/escalation end events publish to `Rodar.Event.Bus` before returning (same pattern as intermediate throw). Compensate end triggers compensation handlers.
...
- **`Rodar.Event.Boundary`** — Full implementation: error (direct activation), message/signal/escalation (event bus), timer (scheduled), conditional (context subscription, fires on data change), compensate (passive — registration in dispatcher), cancel (activated by transaction subprocess on cancel end event). Supports non-interrupting boundary events via `cancelActivity` attribute (boolean, defaults to `true`). Non-interrupting events (`cancelActivity: false`) fire their outgoing path without cancelling the parent activity. Error boundaries are always interrupting per BPMN spec.
- **`Rodar.Activity.MultiInstance`** — Multi-instance activity execution. Wraps any activity handler to execute it multiple times based on BPMN `multiInstanceLoopCharacteristics`. Supports parallel (`isSequential: false`, concurrent via `Task.async_stream`) and sequential (`isSequential: true`, via `Enum.reduce_while`) modes. Completion conditions evaluated after each instance for early termination. Data isolation per iteration via `loopCounter` and optional `inputDataItem`/`outputDataItem` variables. Integrates transparently — the `Rodar` dispatcher checks for `:loop_characteristics` on element attrs before dispatching to the inner handler.
...
- **`Rodar.Activity.DataMapper`** — Data I/O mapping for BPMN tasks. `map_inputs/2` resolves input associations from element attrs to a scoped data map (only keys referenced by `dataInputAssociation`). `map_outputs/3` writes result keys back to context through `dataOutputAssociation` mappings. `has_io_spec?/1` checks for `ioSpecification` presence. Opt-in: if no `ioSpecification`, full context data is passed through (backward compatible). Supports `assignment` elements with `from`/`to` for key remapping.
- **`Rodar.Activity.Task.Service`** — Service task execution. Handler resolution priority: (1) inline `:handler` attribute (e.g., injected via `Diagram.load/2` `:handler_map`), (2) `Rodar.TaskRegistry` lookup by task ID, (3) `{:not_implemented}` fallback. Handler modules implement the `Service.Handler` behaviour. Supports BPMN error propagation (`{:bpmn_error, code, msg}` → error boundary routing) and async execution (`{:async, ref}` → `{:manual, _}` with `complete_async/3` to resume). When element has `ioSpecification`, handler receives only mapped inputs and outputs are written through data output associations.
...
- **`Rodar.Activity.Subprocess.Event`** — Event subprocess handler. Event subprocesses (`:bpmn_activity_subprocess_event`) live inside a parent process scope but only activate when their start event triggers (message, signal, timer, error, escalation, conditional). `register/2` scans a process definition for event subprocesses and subscribes their start event triggers via `Context.subscribe_event/4` (event bus) or `Context.subscribe_condition/4` (conditional). `activate/3` executes the subprocess: interrupting mode (`isInterrupting: true`, default) swaps the context process and runs exclusively; non-interrupting mode (`isInterrupting: false`) spawns a parallel execution. The parser emits `:bpmn_activity_subprocess_event` when `triggeredByEvent="true"` on a `bpmn2:subProcess`.
- **`Rodar.Engine.Diagram`** — Parses BPMN 2.0 XML via `erlsom`, returns process maps keyed by element ID. Splits `intermediateThrowEvent` → `:bpmn_event_intermediate_throw`, `intermediateCatchEvent` → `:bpmn_event_intermediate_catch`. Emits condition expressions as `{:bpmn_expression, {lang, expr}}`. Parses `collaboration`, `participant`, `messageFlow`, `callActivity`, and `laneSet`/`lane` elements (including nested `childLaneSet`). Extracts `timeDuration`, `timeCycle`, `timeDate` from timer event definitions. Parses `multiInstanceLoopCharacteristics` and `standardLoopCharacteristics` on all task types and subprocesses, storing as `:loop_characteristics` attribute. Lane sets are stored in the process attrs as `:lane_set` (nil when absent). `load/1` parses XML, `load/2` accepts opts including `:handler_map` (map of element ID string → handler module) to inject `:handler` attributes into service task elements at parse time, plus `:bpmn_file`, `:app_name`, and `:discover_handlers` (boolean, default `true`) for convention-based handler auto-discovery via `Scaffold.Discovery`. When discovery is active, discovered handlers are merged with explicit `:handler_map` (explicit wins) and the result includes a `:discovery` key. `export/1` delegates to `Rodar.Engine.Diagram.Export.to_xml/1`.
...
- `lib/rodar/activity/` — Tasks (user, script, service, business rule, send, receive, manual) and subprocesses (embedded, event, transaction, call activity)
...
- `lib/rodar/validation.ex` — Structural validation (10 process rules + lane validation + collaboration validation)
